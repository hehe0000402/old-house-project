...
try {
  // 這裡的金鑰保留為空，程式會自動進入「簡易模式」，確保網頁能正常顯示
  // 未來如果您有自己的 Firebase Key，請填入下方物件中
  const config = {
    apiKey: "", 
    authDomain: "",
    projectId: "",
...
const App = () => {
  ...
  // 3. 串接 Google 表單 (填入您的真實 Entry ID)
        ...
        return (
          <div className="space-y-6 animate-fadeIn text-left">
            <div className="bg-white/90 p-6 rounded-3xl shadow-sm border border-pink-100">
              ...
              <div className="bg-amber-50 p-4 rounded-2xl border border-amber-100 flex gap-2 mb-6">
                <AlertCircle className="text-amber-500 shrink-0 mt-0.5" size={16} />
                <p className="text-xs font-bold text-amber-700">安全性評估之最終結果<span className="text-rose-500 underline ml-1">不影響補助申請</span>，請放心評估。</p>
              </div>
      ...
      case 'apply':
        return (
          <form onSubmit={handleSubmit} className="space-y-6 animate-fadeIn pb-12 text-left">
            <div className="bg-white/90 p-6 rounded-3xl shadow-sm border border-pink-100">
              <h2 className="text-xl font-bold text-gray-800 mb-4 flex items-center gap-2"><ClipboardList size={20} className="text-pink-500" /> 填寫預約申請</h2>
              <div className="space-y-4">
                <input required type="text" placeholder="您的姓名" className="w-full p-4 rounded-2xl bg-pink-50/30 border border-pink-100 focus:outline-none focus:ring-2 focus:ring-pink-300" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} />
                <div className="grid grid-cols-2 gap-3">
                  <input required type="tel" placeholder="電話號碼" className="w-full p-4 rounded-2xl bg-pink-50/30 border border-pink-100 text-sm focus:outline-none" value={formData.phone} onChange={(e) => setFormData({...formData, phone: e.target.value})} />
                  <input type="text" placeholder="LINE ID" className="w-full p-4 rounded-2xl bg-pink-50/30 border border-pink-100 text-sm focus:outline-none" value={formData.lineId} onChange={(e) => setFormData({...formData, lineId: e.target.value})} />
                </div>
                <input required type="text" placeholder="房屋地址" className="w-full p-4 rounded-2xl bg-pink-50/30 border border-pink-100 focus:outline-none" value={formData.address} onChange={(e) => setFormData({...formData, address: e.target.value})} />
                <div className="bg-rose-50 p-4 rounded-2xl border border-rose-100 shadow-inner">
              ...
              <button disabled={isSending} type="submit" className="w-full mt-8 py-4 bg-gradient-to-r from-pink-400 to-rose-400 text-white font-bold rounded-2xl shadow-lg flex items-center justify-center gap-2 active:scale-95 transition-all disabled:opacity-50">
                {isSending ? <Loader2 className="animate-spin" size={18} /> : <Send size={18} />} {isSending ? '處理中' : '送出預約申請'}
              </button>
            </div>
          </form>
        );
      ...
      <nav className="flex justify-center px-4 mb-6 shrink-0">
        <div className="bg-white/50 backdrop-blur-md p-1.5 rounded-3xl border border-pink-100 flex gap-1 shadow-sm w-full max-w-sm">
          {['intro', 'safety', 'apply'].map(t => (
            <button key={t} onClick={() => setActiveTab(t)} className={`flex-1 py-3 rounded-2xl text-sm font-bold transition-all ${activeTab === t ? 'bg-white text-pink-500 shadow-md' : 'text-gray-400'}`}>
      ...
      <main className="px-6 pb-12 max-w-lg mx-auto flex-grow w-full text-center">
        {submitted ? (
          <div className="bg-white p-10 rounded-3xl shadow-xl text-center border border-pink-100 animate-bounceIn">
            ...
            <button onClick={() => setSubmitted(false)} className="mt-6 px-8 py-3 bg-pink-50 text-pink-500 font-bold rounded-xl shadow-sm">返回</button>
          </div>
        ) : renderContent()}
      </main>

      <footer className="py-8 px-6 text-center border-t border-pink-100 mt-auto bg-white/50 text-[10px] text-gray-400">
